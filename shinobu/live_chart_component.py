...
const markerFilterOptions = [
  {{ key: "primary_open", label: "?àÎ≤ÑÎ¶¨Ï? Open" }},
  {{ key: "primary_close", label: "?àÎ≤ÑÎ¶¨Ï? Close" }},
  {{ key: "pair_open", label: "Í≥±Î≤Ñ??Open" }},
  {{ key: "pair_close", label: "Í≥±Î≤Ñ??Close" }},`r`n  {{ key: "order_buy", label: "?§Îß§?? }},
  {{ key: "order_sell", label: "?§Îß§?? }}
];
const markerFilters = {{
  primary_open: true,
  primary_close: true,
  pair_open: true,
  pair_close: true,`r`n  order_buy: true,
  order_sell: true
}};
...
function detailHover(items) {{
  return items.map((item) =>
    [
      item.label || "",
      item.time ? `?úÍ∞Ñ: ${{item.time}}` : "",
      item.price ? `Í∞ÄÍ≤? ${{Number(item.price).toLocaleString()}}` : "",
      item.reason ? `?¨Ïú†: ${{item.reason}}` : "",
...
function filterCloseMarkers(markers, closeKey) {{`r`n  if (!markerFilters[closeKey]) return [];`r`n  return markers;`r`n}}`r`n`r`nfunction loadMarkerFilters() {{
  try {{
...
function startCurrentCandleCountdown(payload) {{
  ...
  countdownTimer = setInterval(() => {{
    ...
    if (nextRemaining <= 0) {{
      ...
      liveCountdownState.statusText = "ÏµúÍ∑º Î¥??ïÏ†ï";
      clearInterval(countdownTimer);
      countdownTimer = null;
    }} else {{
      liveCountdownState.statusText = `?ÑÏû¨ Î¥?ÎØ∏Ìôï??¬∑ ÎßàÍ∞êÍπåÏ? ${{liveCountdownState.remainingText}}`;
    }}
    renderCurrentCandleStatusFromState(liveCountdownState);
  }}, 1000);
}}
...
function buildMainFigure(payload) {{
  ...
  const candleHoverText = payload.candles.map((item) => {{
    ...
    return `?úÍ∞Ñ ${{timeText}}<br>?úÍ? ${{Number(item.o).toLocaleString()}}<br>Í≥†Í? ${{Number(item.h).toLocaleString()}}<br>?ÄÍ∞Ä ${{Number(item.l).toLocaleString()}}<br>Ï¢ÖÍ? ${{Number(item.c).toLocaleString()}}`;
  }});
  return {{
    data: [
      {{
    ...
    layout: {{
      ...
      annotations: [
        {{
          ...
          yref: "paper",
          showarrow: false,
          text: `${{payload.symbolName}} ¬∑ 5Î∂ÑÎ¥â ¬∑ ?§Ï†Ñ Í∞ÄÍ≤?,
  ...
  return {{
    data: [
      ...
      {{
        ...
        hovertemplate: `?úÍ∞Ñ %{{customdata}}<br>${{payload.symbolName}} SCR %{{y:.2f}}<extra></extra>`,
        showlegend: false
      }},
      {{
        type: "scatter",
        ...
        hovertemplate: `?úÍ∞Ñ %{{customdata}}<br>${{payload.pairName || "Í≥±Î≤Ñ??}} SCR %{{y:.2f}}<extra></extra>`,
        showlegend: false
      }}
    ],
    layout: {{
      ...
      annotations: [
        {{
          ...
          yref: "paper",
          showarrow: false,
          text: "Î≥¥Ï°∞ÏßÄ??(???§ÏÑ†: ?àÎ≤ÑÎ¶¨Ï? / Ï£ºÌô© ?êÏÑ†: Í≥±Î≤Ñ??",
