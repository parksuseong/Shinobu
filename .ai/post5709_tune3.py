...
def build_signal(df,post=None):
 ...
 open_sig=(os>=BASE['open_prev_need'])&(cu>=BASE['open_cross_need'])&(df['cci20']>-100)&c9&(up>=BASE['up_slope_need'])&reg
 if post:
  ...
  cond4=(df['obv']>df['obv_ema'])&(df['obv_slope']>=0)
...
def backtest(L,S,idx,post=None):
 lo,lc=build_signal(L,post); so,sc=build_signal(S,post); pos=None; ep=None; hp=None; et=None; eq=1.0; peak=1.0; mdd=0.0; rows=[]
 for ts in idx:
 ...
 tr=pd.DataFrame(rows,columns=['entry','exit','ret']); win=float((tr['ret']>0).mean()*100) if len(tr) else 0.0
...
results=[]
for vol,need,adx_s,pdi_s,macd0,macd_d,fade in product([1.0,1.15],[2,3],[0.1,0.4],[0.0,0.8],[-0.01,0.01],[-0.01,0.0],[False,True]):
 post={'vol':vol,'need':need,'adx_s':adx_s,'pdi_s':pdi_s,'macd0':macd0,'macd_d':macd_d,'fade':fade}
 out=backtest(L,S,idx,post)
 if out['trades']<20: continue
...
for i,(s,p,o) in enumerate(results[:6],1):
 print(f"#{i} score={s:.2f} cum={o['cum']:.2f}% mdd={o['mdd']:.2f}% win={o['win']:.1f}% trades={o['trades']} post={p}")
best=results[0]; p=best[1]; o=best[2]
tr=o['tr'].copy(); tr['exit']=pd.to_datetime(tr['exit']); tr['win']=tr['ret']>0; tr['month']=tr['exit'].dt.to_period('M').astype(str)
mon=tr.groupby('month').agg(trades=('ret','size'),win_rate=('win','mean'),ret=('ret',lambda s:(1+s).prod()-1)); mon['win_rate']*=100; mon['ret']*=100
print('\nBEST_POST', {k:round(v,2) if isinstance(v,float) else v for k,v in o.items() if k!='tr'})
print('POST_PARAMS',p)
print('\nMONTHLY')
