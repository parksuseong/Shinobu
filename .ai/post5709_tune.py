...
def macd(close):
 ef=close.ewm(span=12,adjust=False).mean(); es=close.ewm(span=26,adjust=False).mean(); line=ef-es; sig=line.ewm(span=9,adjust=False).mean(); return line-sig

def obv(close,vol):
 d=close.diff().fillna(0.0); signed=vol.where(d>=0,-vol); return signed.cumsum().fillna(0.0)
...
def enrich(df):
 ...
 a,p,m=adx_comp(o); o['adx5']=a; o['pdi5']=p; o['mdi5']=m; o['adx5_slope']=o['adx5'].diff(); o['pdi5_slope']=o['pdi5'].diff(); o['macd_hist']=macd(o['Close']); o['macd_hist_delta']=o['macd_hist'].diff(); o['obv']=obv(o['Close'],o['Volume']); o['obv_ema']=o['obv'].ewm(span=9,adjust=False).mean(); o['obv_slope']=o['obv'].diff(3); vma=o['Volume'].rolling(20,min_periods=5).mean(); o['vol_ratio']=(o['Volume']/vma.replace(0,np.nan)).fillna(0.0)
...
def signal(df,post=None):
 pr=df.shift(1); os=((pr['stoch']<=BASE['st_os']).astype(int)+(pr['cci20']<=BASE['cci_os']).astype(int)+(pr['rsi']<=BASE['rsi_os']).astype(int); cu=((pr['stoch']<=BASE['st_os'])&(df['stoch']>BASE['st_os'])).astype(int)+((pr['cci20']<=BASE['cci_os'])&(df['cci20']>BASE['cci_os'])).astype(int)+((pr['rsi']<=BASE['rsi_os'])&(df['rsi']>BASE['rsi_os'])).astype(int)
 c9=((pr['cci9']<=-100)&(df['cci9']>-100)).rolling(BASE['cci9_window'],min_periods=1).max().astype(bool); up=((df['stoch_slope']>0).astype(int)+(df['cci20_slope']>0).astype(int)+(df['rsi_slope']>0).astype(int); reg=(df['h1_adx']>=BASE['h1_min'])&df['h1_trend']&(df['h4_adx']>=BASE['h4_min'])&df['h4_trend']
 o=(os>=BASE['open_prev_need'])&(cu>=BASE['open_cross_need'])&(df['cci20']>-100)&c9&(up>=BASE['up_slope_need'])&reg
 if post is not None:
  conds=[]
  conds.append(df['vol_ratio']>=post['vol'])
  conds.append((df['pdi5']>df['mdi5'])&(df['adx5_slope']>=post['adx_s'])&(df['pdi5_slope']>=post['pdi_s']))
  conds.append((df['macd_hist']>post['macd0'])&(df['macd_hist_delta']>=post['macd_d']))
  conds.append((df['obv']>df['obv_ema'])&(df['obv_slope']>post['obv_s']))
  score = conds[0].astype(int)+conds[1].astype(int)+conds[2].astype(int)+conds[3].astype(int)
  o = o & (score>=post['need'])
 cd=((pr['stoch']>=BASE['st_ob'])&(df['stoch']<BASE['st_ob'])).astype(int)+((pr['cci20']>=BASE['cci_ob'])&(df['cci20']<BASE['cci_ob'])).astype(int)+((pr['rsi']>=BASE['rsi_ob'])&(df['rsi']<BASE['rsi_ob'])).astype(int); dn=((df['stoch_slope']<0).astype(int)+(df['cci20_slope']<0).astype(int)+(df['rsi_slope']<0).astype(int); crash=(dn>=BASE['down_slope_need'])&((df['stoch']>=BASE['st_ob']-5)|(df['rsi']>=BASE['rsi_ob']-5)|(df['cci20']>=BASE['cci_ob']-15)); c=(cd>=BASE['close_cross_need'])|crash
 if post is not None and post['fade']:
...
def bt(L,S,idx,post=None):
 ...
 tr=pd.DataFrame(rows,columns=['entry','exit','ret']); win=float((tr['ret']>0).mean()*100) if len(tr)>0 else 0.0
 return {'cum':(eq-1)*100,'mdd':mdd*100,'win':win,'trades':len(tr),'tr':tr}

L=load('122630'); S=load('252670'); idx=L.index.intersection(S.index); L=enrich(L.loc[idx]); S=enrich(S.loc[idx])
base=bt(L,S,idx,None)
print('BASE', {k:round(v,2) if isinstance(v,float) else v for k,v in base.items() if k!='tr'})

results=[]
for vol,need,adxs,pdis,macd0,macdd,obvs,fade in product([1.0,1.1,1.2],[2,3],[0.1,0.3,0.5],[0.0,0.5,1.0],[ -0.02,0.0,0.02],[ -0.02,0.0],[ -10000,0,10000],[False,True]):
 post={'vol':vol,'need':need,'adx_s':adxs,'pdi_s':pdis,'macd0':macd0,'macd_d':macdd,'obv_s':obvs,'fade':fade}
 out=bt(L,S,idx,post)
 score=out['cum']-0.35*out['mdd']+0.08*(out['win']-50)+0.02*min(out['trades'],120)
 # reject degenerate too few trades
 if out['trades']<20:
  continue
 results.append((score,post,out))
...
best=results[0]
post=best[1]; out=best[2]
tr=out['tr'].copy(); tr['exit']=pd.to_datetime(tr['exit']); tr['win']=tr['ret']>0; tr['month']=tr['exit'].dt.to_period('M').astype(str)
mon=tr.groupby('month').agg(trades=('ret','size'),win_rate=('win','mean'),ret=('ret',lambda s:(1+s).prod()-1)); mon['win_rate']*=100; mon['ret']*=100
print('\nBEST_POST', {k:round(v,2) if isinstance(v,float) else v for k,v in out.items() if k!='tr'})
print('POST_PARAMS', post)
print('\nMONTHLY')
