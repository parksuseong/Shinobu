...
def signal(df: pd.DataFrame):
    pr = df.shift(1)
    os = (
        (pr['stoch'] <= P['st_os']).astype(int)
        + (pr['cci20'] <= P['cci_os']).astype(int)
        + (pr['rsi'] <= P['rsi_os']).astype(int)
    )
    cu = (
        ((pr['stoch'] <= P['st_os']) & (df['stoch'] > P['st_os'])).astype(int)
        + ((pr['cci20'] <= P['cci_os']) & (df['cci20'] > P['cci_os'])).astype(int)
        + ((pr['rsi'] <= P['rsi_os']) & (df['rsi'] > P['rsi_os'])).astype(int)
    )
    ...
    cd = (
        ((pr['stoch'] >= P['st_ob']) & (df['stoch'] < P['st_ob'])).astype(int)
        + ((pr['cci20'] >= P['cci_ob']) & (df['cci20'] < P['cci_ob'])).astype(int)
        + ((pr['rsi'] >= P['rsi_ob']) & (df['rsi'] < P['rsi_ob'])).astype(int)
    )
    dn = (
        (df['stoch_slope'] < 0).astype(int)
        + (df['cci20_slope'] < 0).astype(int)
    ...
    crash = (dn >= P['down_slope_need']) & (
        (df['stoch'] >= P['st_ob'] - 5)
        | (df['rsi'] >= P['rsi_ob'] - 5)
        | (df['cci20'] >= P['cci_ob'] - 15)
    )
...
def run(no_stop: bool, no_trail: bool):
    L = load('122630')
    S = load('252670')
    ...
    for ts in idx:
        ...
        if pos == 'L':
            high = max(high, lp)
            stop_hit = False if no_stop else (lp <= entry * (1 - P['stop']))
            trail_hit = False if no_trail else (lp <= high * (1 - P['trail']))
            sw = bool(so.at[ts])
            close = bool(lc.at[ts]) or stop_hit or trail_hit or sw
            if close:
                r = (lp / entry) - 1
                eq *= (1 + r)
                peak = max(peak, eq)
            ...
            high = max(high, sp)
            stop_hit = False if no_stop else (sp <= entry * (1 - P['stop']))
            trail_hit = False if no_trail else (sp <= high * (1 - P['trail']))
            sw = bool(lo.at[ts])
            close = bool(sc.at[ts]) or stop_hit or trail_hit or sw
            if close:
                r = (sp / entry) - 1
                eq *= (1 + r)
                peak = max(peak, eq)
...
print('base           ', run(no_stop=False, no_trail=False))
print('no_stop_only   ', run(no_stop=True, no_trail=False))
print('no_stop_noTrail', run(no_stop=True, no_trail=True))
