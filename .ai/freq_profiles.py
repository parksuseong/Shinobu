...
def adx_comp(frame: pd.DataFrame, n: int = 14):
    ...
    pdi = 100 * (pdm.ewm(alpha=1/n, adjust=False, min_periods=n).mean() / atr.replace(0, np.nan))
    mdi = 100 * (mdm.ewm(alpha=1/n, adjust=False, min_periods=n).mean() / atr.replace(0, np.nan))
...
def enrich(df: pd.DataFrame) -> pd.DataFrame:
    ...
    ef = o['Close'].ewm(span=12, adjust=False).mean()
    es = o['Close'].ewm(span=26, adjust=False).mean()
    line = ef - es
    sig = line.ewm(span=9, adjust=False).mean()
    o['macd_hist'] = line - sig
    o['macd_hist_delta'] = o['macd_hist'].diff()

    d = o['Close'].diff().fillna(0.0)
    signed = o['Volume'].where(d >= 0, -o['Volume'])
    o['obv'] = signed.cumsum().fillna(0.0)
    o['obv_ema'] = o['obv'].ewm(span=9, adjust=False).mean()
    o['obv_slope'] = o['obv'].diff(3)

    vma = o['Volume'].rolling(20, min_periods=5).mean()
    o['vol_ratio'] = (o['Volume'] / vma.replace(0, np.nan)).fillna(0.0)
    ...
    for label, rule in [('h1', '60min'), ('h4', '240min')]:
        ...
        aa, pp, mm = adx_comp(tf)
        tf[f'{label}_adx'] = aa
        tf[f'{label}_trend'] = (tf['Close'] >= tf['ema20']) & (pp >= mm)
        o = pd.merge_asof(
...
def build_signal(df: pd.DataFrame, p: dict):
    pr = df.shift(1)
    os = (
        (pr['stoch'] <= BASE['st_os']).astype(int)
        + (pr['cci20'] <= BASE['cci_os']).astype(int)
        + (pr['rsi'] <= BASE['rsi_os']).astype(int)
    )
    cu = (
        ((pr['stoch'] <= BASE['st_os']) & (df['stoch'] > BASE['st_os'])).astype(int)
        + ((pr['cci20'] <= BASE['cci_os']) & (df['cci20'] > BASE['cci_os'])).astype(int)
        + ((pr['rsi'] <= BASE['rsi_os']) & (df['rsi'] > BASE['rsi_os'])).astype(int)
    )
    ...
    reg = (
        (df['h1_adx'] >= BASE['h1_min']) & df['h1_trend']
        & (df['h4_adx'] >= BASE['h4_min']) & df['h4_trend']
    )
    open_sig = (
        (os >= BASE['open_prev_need'])
        & (cu >= BASE['open_cross_need'])
        & (df['cci20'] > -100)
        & c9
        & (up >= BASE['up_slope_need'])
        & reg
    )

    cond1 = df['vol_ratio'] >= p['vol']
    cond2 = (df['pdi5'] > df['mdi5']) & (df['adx5_slope'] >= p['adx_s']) & (df['pdi5_slope'] >= p['pdi_s'])
    cond3 = (df['macd_hist'] > p['macd0']) & (df['macd_hist_delta'] >= p['macd_d'])
    cond4 = (df['obv'] > df['obv_ema']) & (df['obv_slope'] >= p['obv_s'])
    score = cond1.astype(int) + cond2.astype(int) + cond3.astype(int) + cond4.astype(int)
    open_sig = open_sig & (score >= p['need'])

    cd = (
        ((pr['stoch'] >= BASE['st_ob']) & (df['stoch'] < BASE['st_ob'])).astype(int)
        + ((pr['cci20'] >= BASE['cci_ob']) & (df['cci20'] < BASE['cci_ob'])).astype(int)
        + ((pr['rsi'] >= BASE['rsi_ob']) & (df['rsi'] < BASE['rsi_ob'])).astype(int)
    )
    dn = (
        (df['stoch_slope'] < 0).astype(int)
        + (df['cci20_slope'] < 0).astype(int)
    ...
    close_sig = (cd >= BASE['close_cross_need']) | ((dn >= BASE['down_slope_need']) & ((df['stoch'] >= BASE['st_ob'] - 5) | (df['rsi'] >= BASE['rsi_ob'] - 5) | (df['cci20'] >= BASE['cci_ob'] - 15)))

    if p['fade']:
        over2 = (((df['stoch'] >= BASE['st_ob']).astype(int) + (df['cci20'] >= BASE['cci_ob']).astype(int) + (df['rsi'] >= BASE['rsi_ob']).astype(int)) >= 2)
        close_sig = close_sig | (over2 & (dn >= 2) & ((df['macd_hist_delta'] < 0) | (df['obv'] < df['obv_ema'])))

    return open_sig.fillna(False), close_sig.fillna(False)

def bt(L: pd.DataFrame, S: pd.DataFrame, idx: pd.DatetimeIndex, p: dict):
    lo, lc = build_signal(L, p)
    so, sc = build_signal(S, p)
    pos = None; ep = None; hp = None; eq = 1.0; peak = 1.0; mdd = 0.0
    trades = []
    ...
    for ts in idx:
        ...
        if pos == 'L':
            hp = max(hp, lp)
            sw = bool(so.at[ts])
            close = bool(lc.at[ts]) or lp <= ep*(1-BASE['stop']) or lp <= hp*(1-BASE['trail']) or sw
            if close:
                r = (lp/ep)-1
            ...
            hp = max(hp, sp)
            sw = bool(lo.at[ts])
            close = bool(sc.at[ts]) or sp <= ep*(1-BASE['stop']) or sp <= hp*(1-BASE['trail']) or sw
            if close:
                r = (sp/ep)-1
                eq *= (1+r); peak = max(peak, eq); mdd = max(mdd, 1-eq/peak)
                trades.append(r)
    ...
    return {'cum': (eq-1)*100, 'mdd': mdd*100, 'win': win, 'trades': int(len(tr))}
...
cands = []
for vol, need, adx_s, pdi_s, macd0, macd_d, obv_s, fade in product(
    [0.85, 0.9, 1.0],
    [1, 2],
    [0.0, 0.05, 0.1],
    [0.0, 0.3],
    ...
    p = {'vol': vol, 'need': need, 'adx_s': adx_s, 'pdi_s': pdi_s, 'macd0': macd0, 'macd_d': macd_d, 'obv_s': obv_s, 'fade': fade}
    r = bt(L, S, idx, p)
    score = r['cum'] + 0.04*r['trades'] - 0.45*r['mdd']
    cands.append((score, p, r))

cands.sort(key=lambda x:x[0], reverse=True)

# Profile A: 공격형 (90~120 trades 선호)
aggr = [x for x in cands if 90 <= x[2]['trades'] <= 120]
# Profile B: 중립형 (70~90 trades 선호)
neutral = [x for x in cands if 70 <= x[2]['trades'] < 90]
